...
func CreateCustomResources(apiclient apiextensionsclient.Interface,
	resources []CustomResource,
	fake bool) error {

	for _, resource := range resources {
		err := createCRD(apiclient, resource)
		if err != nil {
			return err
		}
	}
	if !fake {
		for _, resource := range resources {
			if err := waitForCRDInit(apiclient, resource); err != nil {
				return err
			}
		}
	}
...
//In this function basically call the apiextensions CustomResource struct
//and initialize with the type monitor
...
func waitForCRDInit(apiclient apiextensionsclient.Interface, resource CustomResource) error {
	crdName := fmt.Sprintf("%s.%s", resource.Plural, resource.Group)
